...
class TextUNet(nn.Module):
    ...
    def __init__(
        ...
        self.Embedd_1 = EmbedFC(512, channels[-3])    # 512 is the size of the text embedding coming from the clip model. 
        ...
        # Define linear layers for initial upsampling
        self.Embedd_2 = EmbedFC(512, channels[-4])    # 512 is the size of the text embedding coming from the clip model. 
        ...
        # Define linear layers for initial upsampling
        self.Embedd_3 = EmbedFC(512, channels[-5])    # 512 is the size of the text embedding coming from the clip model. 
    ...
    def forward(self, x: torch.Tensor, text_embedding: torch.Tensor) -> torch.Tensor:
        ...
        text_embedding_1 = self.Embedd_1(text_embedding).view(-1, 64, 1, 1, 1) 
        #print(text_embedding_1.shape)
        #torch.Size([1, 64, 1, 1, 1])
        
        text_embedding_2 = self.Embedd_2(text_embedding).view(-1, 32, 1, 1, 1) 
        #print(text_embedding_2.shape)
        #torch.Size([1, 32, 1, 1, 1])
        
        text_embedding_3 = self.Embedd_3(text_embedding).view(-1, 16, 1, 1, 1) 
        #print(text_embedding_3.shape) 
        ...
        y = self.conv_transpose_2(torch.cat([y*text_embedding_1, x2], dim=1))
        #print(y.shape)
        #torch.Size([4, 32, 24, 24, 24])

        y = self.conv_transpose_3(torch.cat([y*text_embedding_2, x1], dim=1))
        #print(y.shape)
        #torch.Size([4, 16, 48, 48, 48])
        
        y = self.conv_transpose_4(torch.cat([y*text_embedding_3, x0], dim=1))
        #print(y.shape)
